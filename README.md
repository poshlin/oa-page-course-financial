...
| 頁面聲稱 | 來源 |
|---|---|
| **平台實錄影片 ×3**（`assets/vid-*.mp4`＋poster） | 2026-08-18 由本對話直接驅動已登入的 staging Chrome 錄製（全程未輸入帳密），ffmpeg 裁切內容區。單檔 28–183KB、合計 ~890KB；VideoObject schema ×5 同步（contentUrl 暫指 GitHub Pages，**部署正式站時要跟 og:image 一起改回**）。錄製過程實際操作了平台：存款 11,000（儲蓄 $1,001→$12,001）、簽到 +1,600、命運卡 +3,000——**皆為模擬金幣**。🔴 誤刪教訓：清孤兒資產前必先 grep 引用 |
| **平台實拍截圖**（`assets/shot-*.webp`） | 2026-08-17 由影片對話在 staging 擷取（`~/Downloads/oa-coin-quest-video/capture/stills/`），帳號「小明」、乾淨無瀏覽器外框。🔴 **這是頁面上第一批真實產品畫面**，此前所有平台畫面都是我用 CSS 造的 mockup。`shot-map`＝**「體驗課：支出與公益」18 關**（2026-08-18 由本對話直接操作已登入 Chrome 擷取，取代原第 1 堂版本——家長看到的地圖就是試聽當天那一堂）；`shot-invest`＝投資模組（畫面自帶「共 1370 筆」「匯率 1 台幣＝1 金幣」）；`shot-savings`＝銀行儲蓄（畫面自帶「每雙週利率為 0.1%」＝頁面宣稱的視覺證據）；`shot-insurance`＝醫療險保單（理賠 $3,000 正好對上命運卡 −3,000／+3,000 故事）；`shot-checkin`／`shot-stagemap` 備用未上頁。🔴 商城卡**刻意不換**——現有三張造型（含價格與等級門檻）已是真素材且銷售資訊更強；等級條在 333px 流程卡內文字不可讀，亦不放 |
